#!/bin/bash
# Re-run rate-limited sessions with correct fixture handling
...
while IFS= read -r session_dir; do
    ...
    # Clear old session output and prepare new one
    > "$session"

    # Read prompt
    prompt=$(cat "$prompt_file")

    # Run Claude session directly with local execution
    (
        cd "$workspace"
    ...
    if [ $((rerun_count % 2)) -eq 0 ]; then
        echo "  Waiting 15s before next batch..."
        sleep 15
    fi
