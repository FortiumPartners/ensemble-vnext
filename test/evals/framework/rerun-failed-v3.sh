#!/bin/bash
# Re-run rate-limited sessions with correct fixture handling and absolute paths
...
while IFS= read -r session_dir; do
    ...
    # Skip if not rate limited (check for "hit your limit" or empty file)
    ...
    if [ "$size" -gt 0 ]; then
        # Has some content but not rate limit message
        content=$(cat "$session")
        if ! echo "$content" | grep -q "hit your limit"; then
            continue
        fi
    fi
    ...
    # Run Claude session directly with local execution using absolute paths
    (
        cd "$workspace"
    ...
    # Stagger to avoid rate limits (run 2 at a time, then wait)
    if [ $((rerun_count % 2)) -eq 0 ]; then
        echo "  Waiting 20s before next batch..."
        sleep 20
    fi
