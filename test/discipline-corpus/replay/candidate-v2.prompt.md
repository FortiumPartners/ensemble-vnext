...
Allow it if the payload could make it true: any `session_crons` entry, or a
`background_tasks` entry not marked completed or failed that plausibly matches what the
message is waiting on. A match is enough; never require a second mechanism. Also allow it if
the message names a specific background shell process (a task id, PID or log file), because
those do not appear in the payload.
