...
Check these hooks in `.claude/hooks/` (11 total):
...
3. **Status Hook** (`SubagentStop`) — `.claude/hooks/status.js` (active state machine: advances cycle_position in `implement.json` as subagents complete)
4. **Subagent-Discipline Hook** (`SubagentStop`) — `.claude/hooks/subagent-discipline.js` (blocks a subagent's fire-and-forget / deferred-work claims (ScheduleWakeup is unavailable to subagents, so such claims are false by construction); capped consecutive blocks per agent_id as a loop guard (pairs with `.claude/rules/async-discipline.md`))
5. **Async-Discipline Hook** (`Stop`) — `.claude/hooks/async-discipline.js` (blocks fire-and-forget async claims made without real async machinery in flight (pairs with `.claude/rules/async-discipline.md`))
6. **Autonomy-Discipline Hook** (`Stop`) — `.claude/hooks/autonomy-discipline.js` (blocks hedged mid-loop pause offers that violate autonomous-execution discipline (pairs with `.claude/rules/autonomy.md`))
7. **Wiggum Hook** (`Stop`) — `.claude/hooks/wiggum.js` (autonomous loop intercept/continuation, capped at 50 iterations)
8. **Notify Hook** (`Stop`) — `.claude/hooks/notify.sh` (optional outbound notification (`NOTIFY_ON_STOP`) fired every time a session stops)
9. **Session-Context Hook** (`SessionStart`) — `.claude/hooks/session-context.js` (auto-loads in-flight TRD/PRD state from `.trd-state/current.json` into session context)
10. **Runtime-Refresh Hook** (`SessionStart`) — `.claude/hooks/runtime-refresh.sh` (refreshes vendored `.claude/` components already present from a newer installed plugin (present-only, monotonic version gate); see `docs/TRD/runtime-refresh.md`)
11. **PreCompact Hook** (`PreCompact`) — `.claude/hooks/precompact.js` (archives a decision-trail checkpoint to `.trd-state/<feature>/session-log.md` before context compaction)
