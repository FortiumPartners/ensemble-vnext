...
# 3. Check if NOTIFY_ON_STOP is set
# 4. Execute notification command
# 5. On failure, execute fallback
# 6. Always return {"continue": true}
# Arguments:
#   None
# Returns:
...
main() {
    ...
    # 3. Check if NOTIFY_ON_STOP is set and non-empty
    ...
    # 4. Execute the notification command
    ...
    # 5. Primary command failed, try fallback
    ...
    # 6. Always return success (non-blocking hook)
    output_result
    exit 0
}
