...
component, hook, store, context, style system or client-side model, assume this problem has been met here before and go find where. The engineer who
...
to what you are about to create: a private helper deserves a glance, a shared component, a global store or a design-token change deserves real
looking.
...
Also installed and available whenever the task calls for them: `pytest`, `developing-with-python`, `test-detector`.
