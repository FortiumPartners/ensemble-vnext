...
| 1.2.0 | 2026-08-15 | `/audit-trd` pass. **Citations:** four block-range anchors into `cmd_post` were ~10 lines short and are corrected in all four places they appear (D12, §3.2's error table, CPUB-B005, CPUB-B006) — publisher-init failure `:2579-2587`, `update_draft_status` failure `:2592-2600`, wrong-state guard `:2455-2483`, daily-limit JSON block `:2545-2574`; three single-token anchors corrected alongside them. The behavior described at every one was already right; only the line numbers drifted. AC-F2.5 added to §2.5's leg diagram and NFR-5/AC-N5 to §1.4's sanitizer row so both resolve where the objective index looks for them. **Provenance:** AC-F4.4's inherited *"enforced atomically"* clause verified **false** of `cmd_post`'s guard and recorded as a correction in §1.4 (the requirement itself is unaffected); NFR-5's `request_data` clause verified **inapplicable** — no Python write path exists — and §6.3 narrowed accordingly. **No requirement was removed and no objective lost coverage.** Could Not Verify rewritten to post-audit state: 5 rows kept with out-of-scope reasons, 4 resolved out, 1 unresolvable row added | `/audit-trd` |
...
| D12 | Error classification from the CLI | **Payload first, exit code never.** If stdout parses as JSON, the payload decides the leg outcome *regardless of the exit code*. Only when stdout has no parseable JSON does the coordinator fall back to `failed` with the sanitized stderr tail | AC-F3.2, AC-F7.2, AC-F1.4 | **Corrected in v1.1.0 — `cmd_post` has three exit shapes, not two.** (i) *JSON + `EXIT_OK`*: the normal success/failure payload (`cli.py:2622-2672`, the `TR15` comment), and also **not-found** (`cli.py:2429-2452`) and **wrong-state** (`cli.py:2455-2483`), which both print full JSON and return `EXIT_OK` — the previous draft wrongly filed these as stderr-only. (ii) *JSON + non-zero exit*: the **daily-limit** guard prints a JSON object to stdout **and** returns `EXIT_USER_ERROR` (`cli.py:2545-2574`) — a shape the old two-shape table had no row for, and the reason the exit code cannot gate parsing. (iii) *stderr only, non-zero exit, no JSON*: dedup guard (`:2492-2503`), media guard (`:2505-2516`), platform-not-found (`:2521-2529`), publisher-init failure (`:2579-2587` — how a live Reddit leg fails, `publishers/__init__.py:76-80`), and `update_draft_status` failure (`:2592-2600`). This is also what makes AC-F1.4 work without name special-casing: a Reddit leg is just a leg whose publisher refuses. | (a) Trust the exit code — rejected, it is 0 on publish failure *and* non-zero on a fully-classified daily-limit result. (b) Special-case Reddit in the coordinator — rejected, violates AC-F1.4. **Revisit** if `cmd_post` is ever made to emit JSON on all exit paths. |
...
- **Careful:**
  ...
  - `cmd_post` requires `draft["status"] == "approved"` at the moment it reads the draft
    (`cli.py:2455`) `[read]`; it then claims it via `update_draft_status(id, 'publishing')`
    (`cli.py:2593`) `[read]`. Nothing between the coordinator's D9 transition and that read
...
- **Careful:**
  - **`cmd_post --json` emits `error_category` values outside the six-value taxonomy.**
    ...
    `"wrong_state"` (`cli.py:2477`), `"system_error"` (`cli.py:2390`, `:2418`) `[read]`. None
  ...
  - **§3.2's error table mis-classifies two paths.** `wrong state` and `not found` DO emit
    ...
    (`cli.py:2429-2452` and `:2455-2483`) `[read]`. Genuinely JSON-less paths are: dedup guard
    (`cli.py:2492-2503`), media guard (`:2505-2516`), platform-not-found (`:2521-2529`),
    publisher-init failure (`:2579-2587`), and the `update_draft_status` failure
    (`:2592-2600`) `[read]`. The **daily-limit** guard is a third shape the table does not
    have: it prints JSON to stdout **and** returns `EXIT_USER_ERROR` (`cli.py:2545-2574`)
  ...
  - The success/failure payload keys are fixed at `cli.py:2622-2672`: `success`, `stub`,
    ...
    at `cli.py:2665-2667` states this explicitly `[read]`, confirming D12.
  - A live Reddit leg fails at `publishers/__init__.py:76-80` — `raise ValueError("Reddit
    ...
    `EXIT_SYSTEM_ERROR` (`cli.py:2579-2587`) `[read]`. AC-F1.4's "no name special-casing"
    holds.
...
**Verification state as of v1.2.0 (`/audit-trd` pass, 2026-08-15).** Every row below is
either still unverified or was checked and left standing. What the audit *did* settle has
been moved out of this section and into the body, and is listed at the bottom so it is not
re-raised.

The audit's method sets the boundary: it re-read Herald's code at `/Users/james/dev/herald`
and re-read the PRD. It ran nothing — no dashboard, no database, no network. So every claim
below that needs a **running** system or an **external vendor** is out of its scope by
construction, not by oversight.

### Still unverified — needs something the audit could not run

| Claim | Why the audit could not settle it | How to check it |
|-------|-----------------------------------|-----------------|
| A detached `spawn(..., {detached:true, stdio:'ignore'}).unref()` from SvelteKit survives the request's lifetime (TR5 assumes it may not in `vite dev`; D8 assumes it does in production) | Requires a running dashboard and an observed process lifetime. The audit read code only — and there is no existing detached spawn in the checkout to read, since this feature introduces the first one | Run the create route against a running dashboard and confirm the coordinator completes after the response is sent, in both `vite dev` and `vite preview` |
| `os.kill(pid, 0)` liveness (D6) is adequate against PID reuse on macOS for a single-user local system | Not a code fact. It is a probability argument about a race window, which reading source cannot settle either way | Reason about the window explicitly, or add `owner_started_at` and compare against the process start time via `ps -o lstart` |
| `better-sqlite3` executing the new `CREATE TABLE ... REFERENCES publish_log(id)` DDL succeeds with `foreign_keys=ON` against an existing production `broadcast.db` (D7) | Requires executing DDL against a real database file. The audit confirmed only what the previous pass did — that both runtimes exec `schema.sql` — not that these specific statements pass there | Run `getDb()` against a copy of the production database and assert both tables exist |
| The PhantomBuster API offers no container-scoped output fetch (PRD R3) | External vendor documentation, inherited unresolved from PRD R3. Nothing in this repository can answer it | Read PhantomBuster's `fetch-output`/container API docs, or probe with a real key |
| The LinkedIn Posts API offers no read of recently-created member posts (PRD R4 / OQ-4) | Same — external vendor documentation, inherited unresolved. OQ-4 is owner-only for this reason | Read LinkedIn's Posts API reference for the `w_member_social` scope, or probe with a live token |

### Unresolved by the audit itself

| Claim | What happened | How to close it |
|-------|---------------|-----------------|
| Whether the objective→section index used to audit this document resolves correctly for **AC-F2.5**, **AC-F6.2** and **NFR-5** | The audit found three index entries pointing at sections that do not tag those objectives (D2 for AC-F2.5 and AC-F6.2; §1.4 for NFR-5). Two were index defects, not document defects — AC-F6.2 is tagged on D5 (§1.2) and in §2.5, which the index omitted. The index is an audit-internal artifact and is not part of this TRD, so it could not be corrected here; the TRD side was made to resolve instead (AC-F2.5 added to §2.5's leg diagram, NFR-5/AC-N5 added to §1.4's sanitizer row) | Re-run `/audit-trd` and confirm the same three entries resolve. If they still do not, the defect is in the index builder, not in this document |

### Settled by the v1.2.0 audit — recorded so they are not re-raised

| Former claim | Outcome |
|---|---|
| `cmd_post`'s daily-limit guard is read-then-check rather than atomic, so AC-F4.4's inherited *"enforced atomically per F016 AC-20"* describes `atomic_increment_daily_count()` rather than the guard `cmd_post` runs | **Confirmed true.** `cli.py:2540` reads `platform_row["daily_count"]`, `:2545` compares it, `:2609` advances it with `increment_platform_count()`. `atomic_increment_daily_count()` (`broadcast_db.py:1033`) has zero callers in `src/` — only `tests/unit/test_broadcast_db.py`. Recorded in §1.4 as a correction to the inherited claim. It does not affect this design: legs are one per platform (`UNIQUE(coordinated_id, platform)`, D1) and cannot race each other for one counter |
| NFR-5's requirement to sanitize `request_data` before every `publish_log` INSERT | **Confirmed inapplicable, not unmet.** `_ALLOWED_PUBLISH_LOG_COLUMNS` (`broadcast_db.py:141-159`) has no `request_data` key and `log_publish()` raises `ValueError` on one, so no Python write path exists for this feature to sanitize. The column is written only from TypeScript (`src/lib/db.ts:758`, `src/lib/server/db.ts:1279`), untouched here. §6.3 now states the narrowing; AC-N5 is verifiable as `error_detail` sanitization only |
| §3.2 / D12 / §9's line anchors for the two JSON-less failure blocks and the wrong-state guard | **Found wrong and fixed.** Publisher-init failure is `cli.py:2579-2587` (was `:2570-2579`), `update_draft_status` failure is `:2592-2600` (was `:2585-2595`), the wrong-state guard is `:2455-2483` (was `:2465-2492`), the daily-limit guard's JSON-plus-non-zero-exit block is `:2545-2574` (was `:2547-2570`). The described *behavior* at each block was correct; only the anchors drifted. Also corrected: the claim read at `cli.py:2593` (was `:2588`), the TR15 comment at `:2665-2667` (was `:2666-2668`), and a `system_error` token at `:2418` (was `:2415`) |
| `HERALD_STUB_ERROR`, named as the fault-injection mechanism in the PRD's verification column for AC-F4.2, does not exist | **Confirmed still absent** — re-grepped `src/` and `tests/` during this audit: zero hits. CPUB-T003's DB-seeded-fixture acceptance stands. If a fault-injection hook is added later, CPUB-T003 may switch to it |
