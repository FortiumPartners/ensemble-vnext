...
| 4 | 2.3 `PreCompact` durability hook ✓ implemented (`precompact.js`) | S | Appends structured checkpoint to `.trd-state/<feat>/session-log.md` (timestamp, phase, in-flight task w/ cycle + retry context, last 5 completions, **Decisions & rationale** stub). ~~PreCompact `additionalContext` instructs post-compact model to re-read the log.~~ **[Corrected 2026-08-23 — false, and this sentence is the origin of the defect fixed in `docs/TRD/precompact-schema.md`: PreCompact has no `additionalContext`, so emitting that envelope failed `Hook JSON output validation` on every compaction. The hook now emits `{}`; the post-compaction re-read instruction lives in `implement-trd.md` §5.4.]** Registered in both `settings.json` files. `implement-trd.md` §5.4 documents the convention. |
