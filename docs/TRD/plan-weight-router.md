...
- **Careful:**
  - **Possible duplicate/unreachable logic — see finding.** PLAN-B004's own task text calls for a *direct* `Skill({skill:"create-prd"})` invocation at the `route:'prd'` exit (matching TRD §2.1's diagram and D8's prose, which shows the PRD branch bypassing `fix-plan.js` entirely). But PLAN-B002 is separately building a `route:'prd'` branch **inside** `fix-plan.js`'s `plan()` with its own acceptance test (`chainSkill: 'create-prd'`, `banner: null`, `notify: false`). If B004 is implemented literally as scoped, `fix-plan.js`'s route:'prd' handling is never called from `plan.md`. Coordinate with whoever implements PLAN-B002 before writing both.
...
- **Careful:**
  - **File collision with PLAN-B002 — see finding.** `packages/core/lib/fix-plan.js` is independently and substantially rewritten by PLAN-B002 (tier → weight+route, new `route:'prd'` branch, `/fix`-string fixes), with **no declared dependency** between PLAN-B002 and PLAN-D001. Both edit the same file (this task's target is specifically the JSDoc line PLAN-B002's task description does not mention). Whichever runs second risks clobbering or silently reintroducing the other's text.
  - **File collision with PLAN-P002 — see finding.** `.claude/commands/amend.md` and `.claude/commands/sweep.md` are written directly by this task's "mirror each into `.claude/`" AND rewritten wholesale by PLAN-P002's `scaffold-project.sh --refresh` (which refreshes every command file present in both trees, confirmed `[read]`/`[ran]` under PLAN-P002's own grounding above). No edge orders these two tasks either.
  - **Content-accuracy risk — see finding.** After a literal rename, `process.md`/`process.md.template`'s "SHORTER PATHS" line and `init-project.md`'s "Commands Available" text both read "...investigate, write a light TRD, audit it" — same unconditional-audit inaccuracy as `router.py`'s hint (PLAN-B005's finding): AC-F2.5 makes the audit stage medium-only.
...
- **Replaces:** N/A directly — this task is a delivery mechanism, not a behavior change. (The thing it is *supposed* to make unreachable, `.claude/commands/investigate.md`, is exactly what the finding below says it will fail to remove.)
- **Follow:** the script's own documented `--refresh` invocation pattern (`scaffold-project.sh:10-22` `[read]`).
- **Careful:**
  - **Cannot be built as specified via `--refresh` alone — see finding.** The script's own header comment (`scaffold-project.sh:1259-1264` `[read]`) states refresh "never deletes one the plugin no longer carries ... Adding or removing components stays `/rebase-project`'s job." `copy_commands()` only visits files present in the *source* tree, so once `investigate.md` is deleted from `packages/core/commands/` (by PLAN-P001), `--refresh` never touches `.claude/commands/investigate.md` and it is left behind. This task's stated acceptance criterion (`.claude/commands/investigate.md` does not exist) needs an explicit `rm`, or `/rebase-project`, in addition to `--refresh`.
  - **Collision with PLAN-D001 — see finding.** `.claude/commands/amend.md` and `.claude/commands/sweep.md` are refreshed by this task's `--refresh` run *and* written directly by PLAN-D001 ("mirror each into `.claude/`"), with no dependency edge between the two tasks.
...
- **Careful:**
  ...
  5. Counting "exactly one COMMAND COMPLETE" — see the buildability finding above; implement via the assistant-message-only extraction pattern already in `project.sh`, not a raw whole-session grep.
...
- **Careful:**
  - **The "Owner-only: yes" cell will not produce `ownerOnly: true` — the mechanism doesn't read that column at all.** Read `trd-parser.js` directly [read]: `mapOpenQuestionColumns` (lines 259–275) maps only `id`/`question`/`assumed` roles from the header row; there is no `ownerOnly` column role. `parseOpenQuestions` (lines 579–616) instead computes `ownerOnly = OWNER_ONLY_RE.test(rawRowText) || OWNER_ONLY_RE.test(headingForTable)` (lines 613–614, exactly as the TRD cites), where `OWNER_ONLY_RE = /owner-only|owner ruling/i` (line 29) and `rawRowText` is the **data row's own cell text** (`row.cells.join(' ')`), not the header. A cell whose literal value is the word "yes" never matches that regex. **This means the template exactly as written in TRD §3.3 will parse every Open Questions row to `ownerOnly: false`, and this task's own required assertion (`ownerOnly: true`) cannot pass against a faithful transcription of that template.** Filed as a top-level finding naming PLAN-P001, PLAN-T003 and PLAN-T004 together, since PLAN-T004's new `<open_question>` end-to-end assertion depends on the same mechanism (`implement-trd.md` Step 3.2, line 383, gates emission on `ownerOnly === true`). Before writing this task's fill()/assertions, either confirm PLAN-P001 changed the template's cell text to literally contain "owner-only" (or a heading that does), or raise this back to the template author — writing the test against the TRD's literal example text will produce a red test for the right reason, not a bug in the test.
...
- **Careful:**
  ...
  - **New `<open_question>` end-to-end assertion — trace the full three-hop chain, not just parser output.** Per `implement-trd.md`: the template's `## Open Questions` table → `trd-parser.js`'s `parseOpenQuestions` (`openQuestions[]` with `ownerOnly`, lines 613–614) → Step 3.2 (`implement-trd.md:381-383`, "For every `openQuestions[]` entry where `ownerOnly === true`: determine which task(s) it [covers]") → Step 3.5's `<open_question>` emission (`implement-trd.md:486`, "emit **only** for the owner-only, unresolved question(s) from Step 3.2 that cover this task"). **This assertion is currently unbuildable against the template exactly as specified in TRD §3.3** — see the `ownerOnly` finding filed against PLAN-P001/PLAN-T003/PLAN-T004 jointly: Step 3.2 gates on `ownerOnly === true`, and the template's "yes"-valued "Owner-only" column cell does not set that flag under the parser's actual regex-based mechanism. Do not write this assertion until that upstream issue is resolved one way or the other, or it will be red for a reason unrelated to this task's own correctness.
