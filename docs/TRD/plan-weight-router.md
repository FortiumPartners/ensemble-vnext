...
| AMEND-001 | Two orphaned test files are committed in .claude/lib/ that copy_libs() deliberately never refreshes, so they drift permanently. fix-plan.test.js now tests the retired tier API against the refreshed weight/route module: 14 Jest failures. (`.claude/lib/fix-plan.test.js`) — promoted from a bug discovery found by implement-trd phase-2 gate review | amendment — no objective recorded |  | None | The discovery no longer reproduces |
| AMEND-002 | The mirror-drift test compares packages/full/agents against .claude/agents, but scaffold-project.sh --refresh injects a generated per-project skills: frontmatter block into every .claude/agents/*.md. That pair can never match after any refresh, so the test fails for anyone who runs the refresh this feature requires. (`test/integration/tests/implement-trd-structure.test.sh:511`) — promoted from a bug discovery found by implement-trd phase-2 gate review | amendment — no objective recorded |  | None | The discovery no longer reproduces |
| AMEND-003 | The CLAUDE.md template init-project installs into EVERY scaffolded project teaches a workflow missing 11 of the 18 commands — including all three shorter paths (/plan, /sweep, /amend) and all four verification commands (/audit-prd, /audit-trd, /audit-build, /verify-build). (`packages/core/templates/CLAUDE.md.template`) — promoted from a gap discovery found by consistency review after /audit-build | amendment — no objective recorded |  | None | The discovery no longer reproduces |
| AMEND-004 | Make `.claude/rules/process.md` and `packages/core/templates/process.md.template` describe the loop that actually runs and list all 18 commands. Three defects, one file pair: the template still lists `code-simplifier` and a phase-scoped `/code-review` as active phase-gate stages plus a feature-scale hardening pass (all three removed 2026-08-18 and 2026-08-28); `process.md`'s numbered Process list contradicts its own prose at :155 by keeping both as per-task steps; and neither lists `/sweep`, so both workflow maps enumerate 17 of 18 commands. Absorbed AMEND-005 and AMEND-006 — same two files, so one agent, not three racing. | amendment — no objective recorded | | None | `grep -c code-simplifier` finds no ACTIVE-stage mention in either file (roster mentions in constitution.md stay, code-simplifier is still one of the 13 agents); both workflow maps list 18 commands including `/sweep`; `process.md`'s numbered list matches its own prose. |
...
- **Touches:** `.claude/lib/fix-plan.test.js` (delete), `.claude/lib/fix-audit.test.js` (delete)
- **Replaces:** nothing behavioural. `copy_libs()` skips `*.test.js` deliberately (`packages/core/scripts/scaffold-project.sh:20` states the reason: a scaffolded project has no test runner wired up), so these two committed mirrors can never be refreshed and drift permanently. DELETE them rather than teaching `copy_libs()` to ship test files — a scaffolded project should not carry tests it cannot run, which is the existing design intent. [read]
- **Careful:** `packages/core/lib/fix-plan.test.js` and `fix-audit.test.js` are the REAL tests and Jest already runs them; deleting the `.claude/lib/` copies loses no coverage. Measured: 15/15 passed on `main`, 14 fail now, because the mirror resync updated `fix-plan.js` to the weight/route API while its stale test still calls `plan({tier:'AUTO'})`. The mirror-drift test already skips `*.test.js` (`implement-trd-structure.test.sh:524`), so deleting these does not affect it.

### AMEND-002

- **Touches:** `test/integration/tests/implement-trd-structure.test.sh`
- **Replaces:** the `SKIP` predicate at `implement-trd-structure.test.sh:524`, which handles `*.test.js` and `test-harness.js` but not generated content. Widen it so the `packages/full/agents` ↔ `.claude/agents` pair tolerates the generated per-project block. [read]
- **Careful:** do NOT resolve this by adding the `skills:` block to `packages/full/agents/*.md`. That block is per-project generated content by design — the 4.1.1 change (`c4962d0`) replaced hardcoded skill preloads with deterministic per-project assignment, and the plugin source cannot know a consuming project's stack. The test's expectation is what is wrong, not the mirror. Keep the rest of the `PAIRS` list and its strictness intact; this widens one predicate, it does not weaken the test.

### AMEND-003

- **Touches:** `packages/core/templates/CLAUDE.md.template`
- **Reuse:** this repository's own `CLAUDE.md` "Development Workflow" block as the model for what a current workflow map looks like — it already carries `/plan`, `/sweep` and `/amend` with the whose-plan-not-size distinction. [read]
- **Replaces:** the template's workflow block at lines 36-42, which lists 7 commands and teaches a pipeline three releases stale.
- **Careful:** this file ships to EVERY scaffolded project via `scaffold-project.sh:1421`, so it is the highest-leverage doc in the tree and the one nobody reads here. Eleven of the 18 commands are absent: `/plan`, `/sweep`, `/amend`, `/audit-prd`, `/audit-trd`, `/audit-build`, `/verify-build`, `/update-project`, `/cleanup-project`, `/rebase-project`, `/augment-trd-figma`. It is 93 lines of which 30 are "How to talk to the owner" — keep that section; it is deliberate. Do not simply paste this repo's CLAUDE.md over it: the template is for a project that has not been built yet, so it must stay generic and keep its `{{PLACEHOLDER}}` conventions.

### AMEND-004

- **Touches:** `.claude/rules/process.md`, `packages/core/templates/process.md.template`
- **Reuse:** `.claude/rules/process.md:155`'s own corrected prose — it already states, accurately, that the per-phase `code-simplifier` stage went 2026-08-18 and the per-phase code review 2026-08-28. The fix is to make the rest of both files agree with that sentence. [read]
- **Replaces:** `process.md.template:146,157` (the retired `verify-app --> code-simplifier --> phase-scoped /code-review high` gate and the `feature-scale hardening pass`); `.claude/rules/process.md:163-171` (the numbered Process list that still carries both retired stages as per-task steps).
- **Careful:** `code-simplifier` legitimately remains one of the 13 agents — the roster rows in `constitution.md:185`, `constitution.md.template:109` and `init-project.md:412` are CORRECT and must not be touched. Only the ACTIVE-STAGE descriptions are stale. Both files must be edited in the SAME pass so no transient mismatch exists, the same discipline `notify-on-complete.test.sh`'s `diff -q` pairs enforce for the other rule files. `/sweep` is missing from both workflow maps and is one of the 18.
