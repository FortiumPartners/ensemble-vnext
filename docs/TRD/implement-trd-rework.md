...
| ITR-B013 | Replace `verify-trd-team.md` (842 lines) with `/verify-build` — a deterministic E2E gate that runs the tests and convenes no agent to interpret them | AC-F14.2, AC-F14.3, AC-F14.4, AC-F14.5, D15 | | ITR-B011 | The gate invokes `test/smoke/run-smoke.sh` rather than reimplementing a runner, and reports its exit status; no `Agent` invocation in the gate path; the original command is removed or reduced to a pointer on ITR-B012's terms, with the same seven-array reconciliation; `.claude/rules/command-status.md`'s four `/verify-trd-team` worked examples are updated or the rule file is left documenting a command that no longer exists; mirrored to `.claude/` |
...
|----|----------|----------------------------------|----------------------------------|
| OQ-1 | `trd-authoring.md` defines no "Reference Documents" or "Design References" section, so AC-F6.1's "a section that exists in a generated TRD" has no canonical target. Should the **producer contract** gain one? | This is a scope trade-off across the producer/consumer boundary, and the evidence supports two reasonable readings. Adding the section makes AC-F6.1 true for every future TRD but puts a section into *every* TRD to serve UI tasks that most TRDs do not have; not adding it means `<design_references>` may be permanently omitted — F6 "fixed" into a no-op. Nothing in the PRD, SPEC.md or the code decides which cost is worth paying | D11: loose heading-text matching anywhere in the TRD, element omitted when no match. No producer change. If the matcher finds nothing across the first few real TRDs, that is the evidence that reopens this |
| OQ-5 | What should the two team-command replacements be called? | Command names are user-facing. The PRD answers the *substance* (AC-F14.5: convene no team; delete-vs-rewrite is an implementation choice) and names nothing. Naming is not derivable from any document here, and it propagates into `process.md`, `CLAUDE.md`, `command-status.md`'s worked examples, the scaffold, and seven hard-coded test arrays — cheap now, expensive after the scaffold ships them. **Verified rather than inherited:** no candidate name for either command appears anywhere in the corpus or the code | `/harden-build` and `/verify-build`, to sit alongside `/audit-build` and read as a family |
