...
| D6 | Execution-model split | `/implement-trd` (command) owns TRD parsing, the graph, phase sequencing, `implement.json`, `--resume`. `implement-phase.js` (workflow) owns one phase and writes no durable state | AC-F16.1, AC-F16.2, AC-F16.5, NFR-9, G8 | Inherited verbatim from the source's 2026-08-16 execution-model decision. `resumeFromRunId` is same-session only; an implement run spans sessions. **Marked v1.6.0:** that same-session premise is the load-bearing one for this whole split, and it is **[inherited, unverified]** — it comes from SPEC:465–468 via the PRD, not from an observed platform behaviour, and it is still an open row in Could Not Verify. The split survives a negative (the command owning cross-session state is harmless if resume turns out broader), but the *forcing* argument does not | A whole-run workflow, or one workflow generated per phase — both rejected in the PRD's §8 table. **Revisit** if workflows gain cross-session resume |
...
| D10 | Review routing | `/code-review high` scoped to the phase diff, started **inside** `implement-phase.js`; one further `/code-review high` over the full branch diff started by the command at end of run. **The phase-scoped review's foreground/background status is NOT decided here** — §1.3's runtime-constraints table records that a script's `agent()` calls are subagents with no attested background variant, so "background" (AC-F8.4 / NFR-4 / AC-N4) is a *requirement this decision cannot yet satisfy or refute*. ITR-P003 settles it; the documented fallback is a foreground `agent()` inside the phase (§3.4), which still satisfies NFR-4's context property because the workflow's return is one phase result either way (AC-F16.7) | AC-F8.3, AC-F8.4, AC-F8.5, AC-F16.4, NFR-4 | Inherited from PRD F8. Phase-scoped reviews are structurally blind to cross-phase integration, which is what the end-of-run pass covers. **Hedged v1.6.0** — v1.5.0 stated "as a background subagent" as a settled choice while §1.3, §3.4 and Could Not Verify all recorded it as unattested; the decision now asserts only the routing (inside the phase, scoped to the phase diff), which is what it actually decides | The R1 contingency route (`claude-code-action@v1` on `synchronize`) — held in reserve, not built. Its route and secret are owner-only (NG11). **Revisit** if ITR-P002 shows `/code-review` is not model-startable here, **or when ITR-P003 returns on background dispatch and on whether a workflow-started agent can invoke the `/code-review` skill at all** |
| D11 | `<design_references>` extraction target | **Reconciled with OQ-1 in v1.6.0.** Match the TRD heading **by text, loosely**, against a match set of **three** strings: `Task Grounding` (the OWNER-CALL target from OQ-1, and the only one a `/create-trd`-produced TRD actually carries), plus `Design References` and `Reference Documents` for hand-authored TRDs that have one. Any heading containing any of the three, anywhere in the document, including inside Appendices. First match wins, in that priority order. Emit the element only when a match succeeds | AC-F6.1, AC-F6.2, AC-F6.3 | `trd-authoring.md` defines no "Reference Documents" section, and section numbers shift as authors renumber. A loose text match survives renumbering; the absent case is explicitly an omission, not a guess. **The v1.5.0 wording searched only for the two strings its own rationale admits never appear in producer output**, so `<design_references>` would have been omitted from every TRD `/create-trd` currently produces — satisfying D11's letter while leaving AC-F6.1 unsatisfiable, and contradicting OQ-1's recorded OWNER-CALL two sections later. Adding `Task Grounding` to the match set is what makes the mechanism and the owner decision the same decision | Add a "Reference Documents" section to `trd-authoring.md` so the number is real — rejected here as a producer change outside this TRD's scope, and one that would put a section in every TRD to serve UI tasks only. **Revisit** if a project's TRDs routinely carry design references and the loose match proves unreliable — see OQ-1 |
...
| D15 | Team-command replacements | **None.** Both jobs move into the loop: the adversarial pass runs as a `parallel()` verifier fan-out inside `implement-phase.js` per phase, and once at feature scale from `/implement-trd` after the last phase; the E2E gate is ITR-T002 (`[LIVE]`) plus the phase gate | AC-F14.1, AC-F14.2, AC-F14.3, AC-F14.4, AC-F14.5 | A command adds nothing either job needs. `/implement-trd` knows when phases are done, so feature-scale scope is reachable in-loop | Two standalone commands (`/harden-build`, `/verify-build`) — rejected v1.3.0 and v1.4.0. **Revisit** if hardening code `/implement-trd` did not build becomes routine; today that is `/code-review high` |
...
<ui_context>
  <!-- F6. CORRECTED. Heading matched by TEXT, not by section number, against
       D11's three-string match set in priority order: "Task Grounding"
       (OQ-1's OWNER-CALL target, and the only one real /create-trd output
       carries), then "Design References", then "Reference Documents".
       Omit the whole element when none matches (AC-F6.3). -->
...
- The owner-only Open Question is **surfaced, not asked**, and surfaced **before dispatch**
  (AC-F5.2). The ordering is structural rather than a rule to remember: the element is part of
  the prompt string the command assembles, so it exists before the `Agent` call is made, and the
  DISPATCHED banner that names it is emitted in the same turn. There is no path on which the
  question reaches the reader after the task has already started. It is written into the
  delegation and into the DISPATCHED banner. Turning it into an `AskUserQuestion` would be exactly the
...
| ITR-B012 | Remove `harden-trd-team.md` (765 lines) and `verify-trd-team.md` (842). Neither is replaced by a command: `implement-phase.js` already runs the hardening agent per phase, and `/implement-trd` runs it once more at feature scale after the last phase, before the end-of-run review | AC-F14.1, AC-F14.3, AC-F14.4, AC-F14.5, D15 | | ITR-B005 | The feature-scale hardening pass runs after the final phase and before the end-of-run `/code-review high`; no `Agent({name:...})` teammate spawn anywhere; both original commands removed or reduced to pointers on the seven-array terms; `.claude/rules/command-status.md`'s four `/verify-trd-team` worked examples updated; mirrored to `.claude/` |
...
| NG9 | Reintroducing unit tests as standalone TRD tasks | Already fixed in `packages/core/contracts/trd-authoring.md:390–425` |
...
- **Reuse:** the rule is already in place and its citation checks out —
  `packages/core/contracts/trd-authoring.md:390` *"**UNIT TESTS ARE NOT TASKS. They are part of
  the implementation task.**"*, with the runnable-phase clause *"Do not collect unit tests into
  a terminal 'Verification' phase"* at :423 [read]. **Corrected v1.6.0** — the quoted strings
  were right and the line numbers were wrong (:348 / :380); the block runs :390–425, and NG9's
  cited range was restated from :344-382 to :390–425 in every place this TRD cites it (D8, NG9,
  OQ-3, here).
- **Replaces:** nothing.
...
|----|----------|----------------------------------|----------------------------------|
| OQ-1 | `trd-authoring.md` defines no "Reference Documents" or "Design References" section, so AC-F6.1's "a section that exists in a generated TRD" has no canonical target. Should the **producer contract** gain one? | **OWNER-CALL — decided: point `<design_references>` at `## 9. Task Grounding`.** | Weighed: no `Reference Documents` or `Design References` section exists in `trd-authoring.md`, so AC-F6.1 cannot point at one. The three candidates were (a) add a new section to the contract, (b) delete `<design_references>` from the delegation template, (c) repoint it at an existing section. Chose (c): `## 9. Task Grounding` already carries per-task file context and is the only section whose content matches what `<design_references>` was reaching for. (a) adds a section nothing populates; (b) loses a real delegation input. **Countermand if** you want the contract to gain a genuine references section — then AC-F6.1 changes to creating it. **Reconciled v1.6.0:** this resolution and D11 said different things — the OWNER-CALL named `## 9. Task Grounding` while D11's mechanism searched only for `Design References` / `Reference Documents`, headings D11's own rationale says no producer-generated TRD has. D11 now carries all three strings with `Task Grounding` first, so the mechanism implements the decision. Still loose heading-text matching anywhere in the TRD, element omitted when nothing matches, no producer change. If the matcher finds nothing across the first few real TRDs, that is the evidence that reopens this |
