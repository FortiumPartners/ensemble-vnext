...
| D11 | Opt-in flag | `/implement-trd --verify`, default off | AC-6, R3 | AC-6 names this outright; D-9a-2 gives the reason (unpriced cost on a 1.0-agents-per-task loop) and the condition for flipping it | **On by default** — rejected until a real run yields a cost figure. **Revisit** is explicit: the first costed run |
...
- `stale` — its mtime is not strictly greater than `sinceSec`. The invariant being enforced is
  **"this artifact was produced by THIS run's verification loop"**, so `sinceSec` is
  ...
  the floor to the loop start rejects exactly those and nothing legitimate, because D2 has
  every iteration re-walk every criterion, so all evidence on a resumed run is freshly
  produced anyway. The `max()` is **not** redundant even though a commit time is normally in
  ...
  per-iteration — see the freshness-floor row in `## Could Not Verify` for what it still does
  not establish.** **This check is weaker than it looks — see the
...
interface VerifyFunctionalArgs {
  ...
  since: number;         // max(HEAD commit time, loop start time), seconds (§3.2)
...
**Flag**: `--verify`. Absent → nothing in this TRD executes, including the
background derive pass (AC-6). The usage block, the `Parse:` line and the Execution Model
diagram all name it.

**`--verify --resume`** (D13): when both are passed and
...
`--verify` keeps its existing meaning (resume the implementation checkpoint) and
`--verify` without a state file starts at iteration 1, so the two flags compose rather
...
2. Read `.claude/verification-notes.md` (or `""`), the `stack.md` / `CLAUDE.md` excerpts, and
   the contract text. Resolve `since` as the **later of** `git log -1 --format=%ct` and this
...
block reads `not run (--verify not set)` rather than being omitted, so its absence
...
| R3 | Cost per cycle makes it unaffordable | Opt-in behind `--verify` (D11). The E2E scenario is registered opt-in rather than in `ALL_SCENARIOS`, so it does not add cost to the default smoke run either |
