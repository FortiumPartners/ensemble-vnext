...
|-------|-------|---------------|------------------------|
| 1 | Foundation (each task ships its own unit tests) | None | 1A, 1B can run in parallel |
| 2 | Implementation (each task ships its own unit tests) | Phase 1 complete | 2A, 2B can run in parallel after API contract |
| 3 | Cross-seam integration + `[LIVE]` end-to-end | Phase 2 complete | 3A, 3B can run in parallel |

**UNIT TESTS ARE NOT TASKS. They are part of the implementation task.**

A task that adds a behaviour ships that behaviour's unit tests. They belong in its acceptance
criteria, not in a `-T###` task of their own — and under a test-first constitution they are
written before the code, inside the same task. Splitting them out double-counts the work and
implies the implementation task legitimately ships untested code.

Two real TRDs generated by this contract did exactly that. One created a "Jest unit suite"
task beside the tasks that wrote the code; the other created three tasks prefixed `Unit:`
while the project's own constitution states *"No production code is written before a failing
test exists for it"* and the implementation tasks' grounding already named the test files
they would touch. The unit tests were in the plan twice.

**What DOES earn its own task:**

- **Integration tests that cross a seam no single implementation task owns.** If the test
  exercises the boundary between two tasks' output, neither task can own it; give it a task,
...
genuinely has none — a library, a hook, a CLI with no service — say so explicitly in Quality
Requirements rather than silently omitting it.

**Do not collect unit tests into a terminal "Verification" phase.** If a terminal phase
contains a unit test, that test is in the wrong place and probably should not be a task at
all.
