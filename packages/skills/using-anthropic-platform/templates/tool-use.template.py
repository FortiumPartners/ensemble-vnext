...
This template provides a production-ready tool use (function calling)
...
Usage:
    python tool-use.py "What's the weather in London?"
"""
...
class ToolResult:
    ...
    def to_content_block(self) -> Dict[str, Any]:
        """Convert to Anthropic tool result content block."""
        if self.success:
...
class ToolRegistry:
    ...
    def get_all_schemas(self) -> List[Dict[str, Any]]:
        """Get Anthropic schemas for all registered tools."""
...
class ToolUseClient:
    """Anthropic client with tool use support."""
    ...
    def __init__(
        ...
        """Initialize the tool use client.
        ...
        while iteration < self.max_iterations:
            ...
            try:
                ...
                for tool_use in tool_uses:
                    ...
                    tool_results.append(result.to_content_block())
...
def create_example_client() -> ToolUseClient:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python tool-use.py <message>")
        print("\nExample tools registered:")
        print("  - get_weather: Get weather for a location")
        print("  - calculate: Evaluate a math expression")
        print("\nExamples:")
        print('  python tool-use.py "What\'s the weather in Tokyo?"')
        print('  python tool-use.py "Calculate 15% of 250"')
        sys.exit(1)
